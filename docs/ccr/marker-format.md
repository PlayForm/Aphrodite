...
A CCR marker is a single parseable string that conveys hash, type, size, mode,
preview, and structured metadata to whoever receives it - an LLM agent or the
retrieve endpoint. This lets the receiver decide whether to retrieve the full
content without having to decompress anything first.
...
| --------------------------------------- | -------------------------------------- | --------------------------------------- |
| Proxy hashes (token/cache mode)         | BLAKE3, first 24 hex chars (96 bits)  | `[0-9a-f]{24}`                        |
| Inline hashes (Python plugin fallback)  | SHA-256, first 24 hex chars           | `[0-9a-f]{24}`                        |
| Short inlines                           | `i:` prefix followed by 6+ hex chars  | `` ^(?:[0-9a-f]{24,}\|i:[0-9a-f]{6,})$ `` |

## Type Enum

Values come from both the Rust proxy's classifier and the Python plugin's
classifier:
...
Both the Rust proxy and the Python plugin generate metadata under the same
rules:
...
Full structured metadata, generated with type-specific keys.
