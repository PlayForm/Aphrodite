...
- Regex: `[0-9a-f]{24}`
- Source: `vendor/headroom/crates/headroom-core/src/ccr/mod.rs:compute_key()` (line 86)
...
- Common keys: `lang`, `fns`, `structs`, `classes`, `imports`, `files`, `adds`, `dels`, `trace`, `msg`, `ln`, `keys`, `entries`, `status`, `level`, `N_errors`
...
PREVIEW_TEXT
```
Preview is first 512 bytes of content appended after the marker on a new line. No structured metadata.

### Token mode marker (proxy.rs:1408, smart_marker)
```
<<<CCR:HASH|TYPE|SIZE|METADATA>>>
```
Full structured metadata generated by `generate_metadata()` with type-specific keys.
