...
Origin: External load-balancers and monitoring systems need a fast, public health check that doesn't require loopback access. The `/health` endpoint is intentionally exempt from loopback enforcement.

Source of truth: `crates/aphrodite/src/proxy.rs:health_check()` (line 1796), `crates/aphrodite/src/main.rs:357`
...
`/health` does NOT probe the upstream API. For upstream health, use `/health/upstream` (loopback-only).
