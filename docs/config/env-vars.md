...
| `APHRODITE_HOME`                                | overrides the Python data dir only | `$HERMES_HOME/aphrodite` (else `~/.hermes/aphrodite`) | Python-side data dir (proxy-stderr.log) + the dylib's directive-materialize target; **not** binaries, `aphrodite.toml`, or `ccr.db` (those resolve from `HERMES_HOME` / `~/.hermes`)                                      |
...
| `HERMES_HOME`                                   | base home for the plugin           | `~/.hermes`                                           | Plugin runtime home base (`$HERMES_HOME/aphrodite/binaries`, `$HERMES_HOME/plugins`); read by `__init__.py`, `layout_check.py`, `download.sh`                                                                             |
