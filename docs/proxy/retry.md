...
Origin: Transient network failures to the upstream LLM API should not fail the entire request. A bounded retry loop with exponential backoff and jitter avoids both immediate failure and thundering-herd retry storms.

Source of truth: `crates/aphrodite/src/proxy.rs:proxy_handler()` retry loop (lines 646-677)
...
This timeout applies to each individual attempt. A single slow request can consume up to `timeout` seconds before the retry mechanism kicks in.
