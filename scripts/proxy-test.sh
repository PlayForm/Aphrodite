#!/bin/bash
# Test headroom proxy is routing correctly
# Usage: bash scripts/proxy-test.sh
...
if 'error' in d:
    ...
    print(f'Tokens: {u.get(\"prompt_tokens\",\"?\")}→{u.get(\"completion_tokens\",\"?\")} total={u.get(\"total_tokens\",\"?\")}')
"
...
if 'error' in d:
    ...
    # Rough estimate: raw prompt ~400 tokens
    raw_est = 400
