...
description: "Cargo upgrade breakpoints for aphrodite + headroom — reqwest features, axum ConnectInfo+fallback, tokio-tungstenite Messages, pyo3 allow_threads, workspace pinning. What breaks and how to fix."
version: 1.0.0
platforms: [macos]
---
...
**Symptom**: `package depends on reqwest with feature rustls-tls but reqwest does not have that feature`

**Fix**: Replace `rustls-tls` with `rustls` in all Cargo.toml files. Check both `[dependencies]` AND `[dev-dependencies]` — they can have different feature strings:
```toml
# Before
...
**Symptom**: `the trait bound fn(State<AppState>, ConnectInfo<...>, ...) -> ... {catch_all}: Handler<_, _> is not satisfied`

**Cause**: `any(catch_all)` with `ConnectInfo<SocketAddr>` extractor fails the Handler trait bound in axum 0.8's stricter `fallback()`.
...
**Symptom**: `mismatched types: expected Bytes, found Vec<u8>` and `expected Utf8Bytes, found String`

**Fix**: All Message variants now use `Bytes`/`Utf8Bytes` instead of `Vec<u8>`/`String`:
```rust
// Before (0.24)
