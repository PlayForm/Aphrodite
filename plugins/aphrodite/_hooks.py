...
def _create_live_container(command, env=None):
    ...
    def _run():
        try:
            ...
            _live_containers[hash_val] = {'status': 'running', 'chunks': chunks, 'command': command, 'pid': proc.pid}
            for line in proc.stdout:
                chunks.append(line)
            proc.wait()
            _live_containers[hash_val]['status'] = 'done' if proc.returncode == 0 else f'exit={proc.returncode}'
            _live_containers[hash_val]['exit_code'] = proc.returncode
        except Exception as e:
            _live_containers[hash_val] = {'status': 'error', 'error': str(e), 'command': command}
    _live_threading.Thread(target=_run, daemon=True).start()
    return f'<<<LIVE:{hash_val}|terminal|streaming>>> [terminal:{command[:40]} ...running]'
...
def _live_container_handler(args=None, **kwargs):
    ...
    return json.dumps({'hash': hash_val, 'status': c.get('status', ''), 'command': c.get('command', ''), 'output': output[-4096:], 'output_len': len(output), 'pid': c.get('pid')}, indent=2)

LIVE_CONTAINER_SCHEMA = {'name': 'aphrodite_poll_container', 'description': 'Poll a live CCR container for streaming output.', 'parameters': {'type': 'object', 'properties': {'hash': {'type': 'string', 'description': 'Hash from <<<LIVE:hash>>> marker'}}, 'required': ['hash']}}
