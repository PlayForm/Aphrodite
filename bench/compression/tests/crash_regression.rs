...
fn utf8_boundary_fixture_straddles_500_byte_marks() {
	...
	let f = pathological_corpus()
		.into_iter()
		.find(|f| f.name == "utf8_boundary.txt")
		.unwrap();
	let bytes = f.content.as_bytes();
	let straddles = (500..bytes.len())
		.step_by(500)
		.filter(|&i| !f.content.is_char_boundary(i))
		.count();
	assert!(
		straddles > 0,
		"no 500-byte offset lands inside a codepoint - fixture lost its teeth"
	);
}
...
fn expand_preserves_literal_markers_byte_identical() {
	...
	let f = pathological_corpus()
		.into_iter()
		.find(|f| f.name == "marker_literal.txt")
		.unwrap();
	...
	assert_eq!(
		aphrodite::resolve::resolve_one(&mut state, "aaaabbbbccccddddeeeeffff"),
		Some(f.content)
	);
}
...
fn interior_nul_content_round_trips_through_inline_store() {
	...
	let f = pathological_corpus()
		.into_iter()
		.find(|f| f.name == "interior_nul.json")
		.unwrap();
