...
**Shape:** Docstring
**Upstream (current pipeline target):** `r"""Wrapper for ...`
**Fork:** `R"""\nAuto-generated by:\n{cmd_str}"""` (`printer_python.py:81`)

---

**Shape:** Preamble
**Upstream (current pipeline target):** embeds UserString/String/ReturnString + loader classes (~250 lines, cf. committed `_bindings.py:30-334`)
**Fork:** only `import ctypes` + `from ctypes import *` (`printer_python.py:82-85`); no String class unless template file supplies one

---

**Shape:** Library binding
**Upstream (current pipeline target):** `_libs["X"] = load_library("X")`
**Fork:** `_libs["X"] = _get_library(name, dllclass = ctypes.CDLL, libpaths = (...), search_sys = ...)` (`printer_python.py:165-188`); `_get_library` at `libraryloader.py:14-34` returns a plain `dllclass(path)` - a raw `ctypes.CDLL`

---

**Shape:** Function binding
**Upstream (current pipeline target):** `for _lib in _libs.values(): if not _lib.has("N","cdecl"): continue; N = _lib.get("N","cdecl")` (loop shape)
**Fork:** top-level `N = _libs['LIB']['N']` (CDLL `__getitem__` subscript) (`printer_python.py:197`)

---

**Shape:** restype lines
**Upstream (current pipeline target):** if/else `ReturnString`/`String`+errcheck block, or indented single line
**Fork:** single top-level line, value `POINTER(c_char)`/`String`/`WideString`/other

---

**Shape:** Templates/loader
**Upstream (current pipeline target):** embedded inline
**Fork:** embedded inline by default; `--no-embed-templates` + `--linkage-anchor` (both mandatory together, `__main__.py:462-465`) generates a shared `_ctg_loader.py` sibling (`printer_python.py:149-162`)
