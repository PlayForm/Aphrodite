...
**Root cause to fix:** on the _explicit_ tool path the caller's `type` hint is the authoritative intent, and the
...
prevents the literal-`ok` preview on the _hook_ path too, where no hint exists to disambiguate.
...
1. **Hook path has the same fragment-preview bug for user JSON** (lib.rs:414 → hooks.rs:172-197): a tool result
   ...
   _recognized envelope signature_ (presence of `exit_code`/`total_count`/`matches`/`diff` keys) before collapsing.
