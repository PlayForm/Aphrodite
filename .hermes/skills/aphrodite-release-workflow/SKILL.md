...
| Field            | Authority path                                                                                                                                                                                                                 | Verified value at `a81acab6` (Development, 2026-09-20)                                                                       |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------- |
| Binary version   | `crates/aphrodite/Cargo.toml` `version`; `crates/aphrodite-hermes/Cargo.toml` `version` **and** its `aphrodite = { path = "../aphrodite", version = "X", ... }` dep pin; `package.json` `version` (carries the binary version) | `1.5.0` in both crates + hermes pin; **`package.json` lags at `1.4.6` - drift; the four must move together in one ceremony** |
| Plugin version   | `plugins/aphrodite/plugin.yaml` `version` + the `install_message` block                                                                                                                                                        | `2.2.0`                                                                                                                      |
| `BINARY_VERSION` | `plugins/aphrodite/BINARY_VERSION` (read by `download.sh` / `download.ps1`)                                                                                                                                                    | `1.5.0`                                                                                                                      |
| Parent gitlink   | `git submodule status plugins/aphrodite`; `git ls-tree HEAD plugins/aphrodite`                                                                                                                                                 | `4ac01b76c8` (described as `v2.1.3-32-g4ac01b7`)                                                                             |
| README badge     | `README.md` release badge, plugin badge, example health output                                                                                                                                                                 | release `v1.5.0`, plugin `v2.2.0` - consistent with the manifests                                                            |
...
- **Two version tracks, never conflated:** binary `1.4.x/1.5.x` (parent
  crates + `package.json` + README badge) vs plugin `2.1.x/2.2.x` (S
