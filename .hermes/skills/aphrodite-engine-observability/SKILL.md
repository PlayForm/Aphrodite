...
version: 1.0.0
platforms: [macos]
tags: [aphrodite, observability, health, probes, cache, telemetry, layers]
status: active
scope:
    repositories:
        - PlayForm/Aphrodite
...
verification:
    source_of_truth:
        ...
        - Maintain/examples/08_health_upstream.py (upstream probe documentation)
...
- The process/session was restarted after a change when needed - a stale
  dylib process can mask source changes (AGENTS.md rule).
...
```json
{ "status": "healthy", "ccr": true, "mode": "token|cache", "version": "1.4.6", "fill_pct": 0.42 }
```
...
- Upstream health is probed SEPARATELY: `GET /health/upstream` (deepseek/
  ...
  `Maintain/examples/08_health_upstream.py`.
...
| Proxy stderr        | `proxy-stderr.log` in plugin data dir; default `~/.hermes/aphrodite/proxy-stderr.log` | `__init__.py:958-977`              |
| SQLite CCR store    | `~/.hermes/aphrodite/ccr.db`                                                          | AGENTS.md                          |
| Runtime home        | `~/.hermes/aphrodite/` (aphrodite.toml, binaries/, directives/, hotreload/)           | AGENTS.md                          |
| Hot-reload evidence | dylib mtime change log + warning that reload wipes session CCR state                  | `__init__.py:531-541`              |
