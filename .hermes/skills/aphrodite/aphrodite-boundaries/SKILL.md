...
platforms: [macos]
category: aphrodite
category_taxonomy: aphrodite/aphrodite-boundaries
date: 2026-09-25
metadata:
    hermes:
        tags: [aphrodite, boundaries, safety, stop-conditions, recovery, git-discipline]
        related_skills: [aphrodite-orientation, aphrodite-compression-safety]
status: active
scope:
    repositories:
        - PlayForm/Aphrodite
...
- Never run git write commands (commit, add, reset, stash, checkout, restore,
  clean) during delegated skill work: the git INDEX is the user's staged
  baseline and must stay untouched - edit working-tree files only. An external
  auto-committer sweeps working-tree changes: unexpected `git status` output is
  expected, never "fix" it with a git write command.
...
- Never bump `BINARY_VERSION` (the pin in `~/.hermes/aphrodite/BINARY_VERSION`)
  before the referenced release assets exist and are accessible.
...
- On a missing key, emit a clear degraded-state message rather than an opaque
  proxy failure.
- Verify `APHRODITE_API_KEY` is actually exported: a var present but commented
  out in the environment file fails the proxy loudly at startup - presence
  checks must test export, never assume.
