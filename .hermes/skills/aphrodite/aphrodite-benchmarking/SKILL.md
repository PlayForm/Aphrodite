...
| Marker validity suffices for throughput/type tests | `aphrodite_test` / `aphrodite_compress` response | Compress one payload per type; validate marker grammar + stats (marker JSON shape: `{"hash":..., "type":..., "size":..., "preview":..., "marker":...}`) | Valid markers; stats move as expected       | Retrieve + compare (resilience path)              |
