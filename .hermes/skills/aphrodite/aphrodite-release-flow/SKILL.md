...
| Tag push publishes `aphrodite` + `aphrodite-hermes` crates (via the Build -> Publish `workflow_run` chain) | Publish.yml `on:` + job gates at the tag commit | `git show <tag>:.github/workflows/Publish.yml \| grep -nE 'workflow_run|head_branch'` | `on:` is `workflow_run` on Build `completed` and both publish steps carry `startsWith(github.event.workflow_run.head_branch, 'Aphrodite/v')` (current state) or the audit records the change | Accept the side effect explicitly or halt the tag |
| `aphrodite-headroom-core` is never tag-published           | Publish.yml Publish-Headroom-Core publish-step `if:`                                            | Read the publish-step `if:` at the tag commit                                               | publish-step `if:` names `workflow_dispatch && publish_crates` - unreachable under the `workflow_run`-only `on:` (dispatch input gone at 677a7b3); headroom-core never CI-published                                                             | Treat any tag-reachable headroom publish as unexpected - stop                   |
