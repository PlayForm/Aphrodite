...
- **Two version tracks, never conflated:** binary `1.6.x` (parent
  ...
  with (what `download.sh`/`download.ps1` fetch). Current verified values:
  binary `1.6.2` (both crates), plugin `2.2.2`, `BINARY_VERSION` `1.6.2` -
  re-derive each from its authority path live, never assume. Canonical
  ledger: `aphrodite-release-workflow` §1 (5 rows, per-row authority /
  earliest-update / latest-safe-update / verification). Historical drift at
  `a81acab6` (2026-09-20): `package.json` lagged at `1.4.6` while both
  crates read `1.5.0` - ledger rows drift; fix the manifest before claiming
  a release and report the drift.
