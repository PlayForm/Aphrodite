...
platforms: [macos]
category: aphrodite
category_taxonomy: aphrodite/aphrodite-auto-expand-testing
date: 2026-09-25
metadata:
    hermes:
        tags: [aphrodite, ccr, auto-expand, testing, debug, retrieve, live-read]
        related_skills: [aphrodite-boundaries, aphrodite-orientation, aphrodite-benchmarking]
status: active
scope:
    repositories:
        - PlayForm/Aphrodite
...
- when the proxy is down or misconfigured, NO markers appear: the startup
  failure "no API key configured - set APHRODITE_API_KEY env var" means the key
  is absent OR commented out in the environment file - verify the var is
  actually exported before debugging marker behavior against the proxy
- dylib-side diagnostics are silent inside the Hermes host: tracing::warn!/
  info! have no subscriber, so "no log output" proves nothing - use a stderr
  fallback (check tracing::dispatcher::has_been_set()) or a surfaced state
  field when debugging marker behavior
- skill content is hash-scanned on load (skills_guard): embed CCR marker
  examples escaped/benign so flagged content does not quarantine this skill
  until a re-scan passes
