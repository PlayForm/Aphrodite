...
platforms: [macos]
category: aphrodite
category_taxonomy: aphrodite/aphrodite-compression-safety
date: 2026-09-25
metadata:
    hermes:
        tags: [aphrodite, ccr, safety, compression, recursion, pairing, fail-open, classify, guard]
        related_skills:
            [
...
Claims below are **verified** (checked in source on the Development branch) or
...
7. **Treat binary, JSON, code, and multiline text as distinct preview
   ...
   shape, never a flat truncation for everything (verified in `proxy.rs`; the
   parallel preview builder was removed).
...
Fail-open logging must actually be visible: inside the Hermes host the dylib
has no tracing subscriber, so `tracing::warn!`/`info!` are silent no-ops -
use a stderr fallback (check `tracing::dispatcher::has_been_set()`) or a
surfaced state field, or the "log the error" half of fail-open is invisible.
...
- Skill content is hash-scanned on load (skills_guard): keep CCR marker
  examples in this skill escaped/benign so flagged content does not quarantine
  the skill until a re-scan passes.

## Reference evidence

- `references/recompression-guard.md` - retrieval-tool skip list history and
  the guard pattern (historical; the plugin is now a pure loader -
  `~/.hermes/plugins/aphrodite` holds only `plugin.yaml` + `__init__.py` and
  the skip logic lives in the dylib under `~/.hermes/aphrodite/binaries/`).
- `references/ccr-infinite-recursion.md` - the infinite-recursion incident
  (historical tool names `headroom_retrieve`/`headroom_stats`; the current
  tool family is `aphrodite_*`).
