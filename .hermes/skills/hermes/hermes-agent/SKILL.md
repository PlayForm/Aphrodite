...
the terminal, a native desktop app, messaging platforms, and IDEs, works with
any LLM provider, and is extensible via plugins, MCP servers, skins, TUI
widgets, and cron.

What makes Hermes different:

- **Self-improving through skills** - Hermes learns reusable procedures as
  skills that load into future sessions (the repo's dev skills live in
  `.hermes/skills/`, Development branch only, never shipped with the plugin).
...
operation, spawning/orchestration, and hard invariants. Everything else lives
in reference files - **load the matching reference (below) before answering**;
do not answer detail questions from the body alone.
...
```
terminal(command="hermes chat -q 'Benchmark the proxy: run aphrodite_test and write the summary to ~/ccr-roundtrip.md'", timeout=300)
...
- **Use `--profile dev-aphrodite` when spawning agents that must see the repo's plugin** - the default profile does not load it
...
- **OpenAI-compatible proxy** (`hermes proxy`) - a local OpenAI API backed by whichever OAuth provider you're signed into. Distinct from the Aphrodite CCR proxy (`crates/aphrodite`), which compresses session context rather than serving an API.
