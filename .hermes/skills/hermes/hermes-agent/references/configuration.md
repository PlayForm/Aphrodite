...
| `terminal`    | `backend` (local/docker/ssh/modal/daytona/singularity), `cwd`, `timeout` (180), `env_passthrough`      |
...
- `terminal.env_passthrough` must include `APHRODITE_API_KEY` (plus `PATH`,
  `HOME`) or the CCR proxy subprocesses start without their key:
  `hermes config set terminal.env_passthrough '["APHRODITE_API_KEY","PATH","HOME"]' --profile dev-aphrodite`
- The proxy's own settings (compression thresholds, hooks, runtime layout)
  live in `~/.hermes/aphrodite/aphrodite.toml`, not in config.yaml's
  `compression` section. Verify with `aphrodite_stats` / `aphrodite_rebuild`.
- Config changes for the dev instance are scoped with `--profile dev-aphrodite`.
