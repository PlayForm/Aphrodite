...
Use when the user wants a live panel in the Hermes TUI (`hermes --tui`),
including an Aphrodite dev session. Author widget apps: glanceable ambient
panels docked above the status bar, or modal overlays that own the keyboard.
Widgets are plain ESM files the TUI loads at startup - no build step, no repo
changes. This reference does not cover desktop-app or web-dashboard widgets.

## When to Use

- The user asks for a live panel in the TUI (ticker, clock, countdown,
  status card, API-backed readout - e.g. a docked panel showing the CCR
  engine's proxy-health counters).
...
1. Use `write_file` to create `$HERMES_HOME/tui-widgets/<name>.mjs` (see
   `templates/clock.mjs` for a complete working widget; under the
   `dev-aphrodite` profile that is
   `~/.hermes/profiles/dev-aphrodite/tui-widgets/`).
...
- `mode: 'ambient'` - captures no input, the command toggles it; `render`
  ...
  every zone RESERVES real space (nothing ever paints over the transcript):
...
**Stop if** a widget never appears and the log shows a thrown `register()`

- the file is the problem, not the dock.

**Recovery** - permitted: fix the `.mjs` and `/widgets-reload`. Prohibited:
moving the file into the repo tree to "make it load" - widgets belong in
`$HERMES_HOME/tui-widgets/`, scratch in `.hermes/tmp/`.
...
| --------------------- | ------------------------------- | ---------------------------------- |
| Widget hot-loads      | Write the `.mjs`, watch the TUI | Appears within ~a second           |
| Reload lists the file | `/widgets-reload`               | `loaded:` names the file           |
| Ambient toggles       | `/<id>` twice                   | Docks on first, unmounts on second |
