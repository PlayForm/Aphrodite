...
Use when the user wants a new desktop UI element in the Hermes app - a pane,
a statusbar widget, a dashboard, a command - without modifying the app
itself, or wants to surface data computed in the gateway inside the app. A
plugin is a single plain-JavaScript ESM file the app loads at runtime - no
build step, no repo changes. A plugin can also talk to its own Python backend
namespace (`ctx.rest`/`ctx.socket` → `/api/plugins/<id>`); the general Python
plugin system (`~/.hermes/plugins/`) is otherwise documented separately.
...
- Write access to `$HERMES_HOME/desktop-plugins/` (usually
  `~/.hermes/desktop-plugins/`; under the `dev-aphrodite` profile that is
  `~/.hermes/profiles/dev-aphrodite/desktop-plugins/`).
...
In an Aphrodite dev setup, run the desktop app against the dev profile
gateway so `host.state.profile` reports `dev-aphrodite` and panes reflect
the dev session.
...
- `host.state.*` - readonly reactive atoms: `activeSessionId`, `busy`,
  ...
  in handlers, `useValue(atom)` in components.
...
**Stop if** a "plugin not appearing" report arrives - check the enable
toggle (opt-in half) and the ⌘K reload before debugging code.

**Recovery** - permitted: flip the toggle, reload plugins, read the error
toast. Prohibited: editing files under `$HERMES_HOME/plugins/<id>/` to
"force" enablement.
...
| ------------------------------ | -------------------------- | ------------------------------------- |
| Plugin loads                   | **Reload desktop plugins** | UI appears; no "failed to load" toast |
| Pane is draggable              | Drag the new zone          | Behaves like a core pane              |
| Opt-in stays off until toggled | Install without enabling   | Inventoried in Settings, not active   |
