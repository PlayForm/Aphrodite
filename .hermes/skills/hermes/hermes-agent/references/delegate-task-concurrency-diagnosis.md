...
Use when a user reports `delegate_task` ran fewer subagents than they asked
for (e.g. "I set max_concurrent_children: 15 but only 9 ran") - including
parallel Aphrodite workflows (crate benches, skill-library sweeps). There are
exactly **three** code paths in Hermes that cap a batch. If none of them
fired, the cap came from the **model itself** - not from Hermes - and the
user's narration of "the runtime caps at N" is the model rationalising its
own choice.
...
# (under the dev profile: add --profile dev-aphrodite)
...
**Stop if** the user "fixes" the cap by lowering `max_concurrent_children`

- the config is not the problem when neither log line fired.

**Recovery** - permitted: re-prompt with an explicit single-call `tasks`
array, or use `execute_code` as a deterministic courier. Prohibited:
blaming the runtime without running the diagnostic recipe.

## Verification

| Claim                       | Test                             | Pass condition                       |
| --------------------------- | -------------------------------- | ------------------------------------ |
| Resolver matches config     | Step 3 of the recipe             | Printed value == `hermes config get` |
| No cap fired                | Neither log line appears         | Cap attribution goes to the model    |
| Single-call batch runs full | One `delegate_task` with N tasks | N children spawned                   |
