/**
 * Aphrodite desktop plugin template. Save as:
 *   <hermes home>/desktop-plugins/aphrodite-proxy/plugin.js   (folder name == id)
 * where <hermes home> is ~/.hermes by default, or ~/.hermes/profiles/<name>
 * when running a named profile (`hermes --profile dev-aphrodite`). Run `hermes doctor`
 * (or check the app's Settings → Plugins folder path) if unsure which is active.
...
const ID = 'aphrodite-proxy'

function ProxyPane() {
  const t = usePluginI18n(ID)
  const proxy = useValue(host.state.gateway)
...
function ProxyChip() {
  ...
  return jsx(Tip, {
    ...
    children: jsx('button', {
      ...
      onClick: () => {
        haptic('tap')
        host.notify({ kind: 'info', message: t('roundtrip') })
      },
      children: 'aphrodite-proxy'
    })
  })
}
...
export default {
  ...
  name: 'Aphrodite Proxy',
  register(ctx) {
    // Register locale bundles - scoped to this plugin, torn down on reload.
    // Values are literals or interpolators (`arg => `…${arg}…``).
    ctx.i18n.register({
      en: {
        paneTitle: 'Aphrodite Proxy',
        proxy: state => `proxy: ${state}`,
        chipTip: 'Aphrodite proxy - click me',
        roundtrip: 'CCR round trip OK'
      },
      ja: {
        paneTitle: 'アフロディーテプロキシ',
        proxy: state => `プロキシ: ${state}`,
        chipTip: 'アフロディーテプロキシ - クリック',
        roundtrip: 'CCR ラウンドトリップ OK'
      }
    })
    ...
    ctx.register({
      id: 'pane',
      area: 'panes',
      title: 'aphrodite proxy',
      data: { placement: 'right', width: '237px' },
      render: () => jsx(ProxyPane, {})
    })
