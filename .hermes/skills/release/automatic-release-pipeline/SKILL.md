...
- Aphrodite releases must be cut on a recurring schedule with no manual step per dependency update; the only manual gap is bump version + write CHANGELOG + create the GitHub Release (the "crate + npm wrapper generating release notes" exists as git-cliff)
