...
//! These match exactly what the Python plugin's `_tools.py` and
//! `_hooks/__init__.py` export.
...
fn schema_reclassify() -> serde_json::Value {
	json!({
		...
		"parameters": {
			"type": "object",
			"properties": {
				"hash": {"type": "string", "description": "Optional: reclassify a single entry by hash. Omit to reclassify all entries."}
			}
		}
	})
}
