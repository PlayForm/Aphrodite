...
pub fn filter_lines(content: &str, query: &str) -> String {
    ...
    if matching.is_empty() {
        format!(
            "[aphrodite: no lines matched {query:?} — returning full content]\n{content}"
        )
    } else {
        matching.join("\n")
    }
...
mod tests {
    ...
    fn test_resolve_one_i_prefix() {
        ...
        assert_eq!(
            resolve_one(&mut s, "i:abc123def"),
            Some("inline content".into())
        );
    }
    ...
    fn test_expand_unresolved() {
        ...
        s.inline_store_put(
            "outer".into(),
            "before <<<CCR:missing|text|10>>> after".into(),
        );
        let result = expand(&mut s, "outer");
