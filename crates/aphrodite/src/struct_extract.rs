...
fn sig(kind:&str, text:&str) -> String {
	...
	if s.len() > MAX_SIG_LEN { s[..MAX_SIG_LEN - 3].to_string() + "..." } else { s }
}
	...
	for line in content.lines() {
		...
		if is_fn {
			...
			if let Some(paren) = after_fn.find('(') {
				...
				let s = if s.len() > MAX_SIG_LEN { s[..MAX_SIG_LEN - 3].to_string() + "..." } else { s };
				let slen = s.len();
				fns.push(s);
	...
	for line in content.lines() {
		...
		if is_def {
			...
			if let Some(paren) = after.find('(') {
				...
				let s_trunc = if s.len() > MAX_SIG_LEN { s[..MAX_SIG_LEN - 3].to_string() + "..." } else { s };
...
fn extract_js(content:&str, result:&mut HashMap<String, Vec<String>>, budget:&mut isize) {
	...
	for line in content.lines() {
		...
		if trimmed.contains("=>") && trimmed.contains('=') && !trimmed.starts_with("//") {
			...
			if before_eq.starts_with("const ") || before_eq.starts_with("let ") || before_eq.starts_with("var ") {
				let name = before_eq
