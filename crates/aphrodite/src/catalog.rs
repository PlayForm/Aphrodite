...
pub fn build_catalog(state: &AphroditeState, mode: &str) -> serde_json::Value {
    let items: Vec<serde_json::Value> = state.recent_markers.iter().map(|m| {
        if mode == "toc" {
            serde_json::json!({
    ...
    for m in &state.recent_markers {
        by_type.entry(m.ccr_type.clone())
            .or_default()
            .push(m.hash.clone());
    }

    let by_type_json: serde_json::Map<String, serde_json::Value> = by_type.iter()
        .map(|(t, hashes)| {
    ...
    if mode == "toc" && !items.is_empty() {
        let recommendations: Vec<String> = state.recent_markers.iter()
            .take(10)
...
pub fn format_catalog_table(state: &AphroditeState) -> String {
    ...
    let mut lines = vec![
        format!("Catalog: {} items {} saved {} turns {} files",
            items.len(),
...
mod tests {
    ...
    fn test_catalog_with_items() {
        ...
        s.record_marker(crate::state::MarkerEntry {
            hash: "abc123def456".into(), ccr_type: "code_rust".into(),
            size: 1024, preview: "[code:3fns 42L]".into(),
            turn: 1, center: None, meta: None,
        });
        s.record_marker(crate::state::MarkerEntry {
            hash: "def789abc012".into(), ccr_type: "diff".into(),
            size: 2048, preview: "[diff:1F +2/-1 10L]".into(),
            turn: 1, center: None, meta: None,
        });
    ...
    fn test_toc_mode() {
        let mut s = AphroditeState::default();
        s.record_marker(crate::state::MarkerEntry {
            hash: "abc".into(), ccr_type: "text".into(),
            size: 100, preview: "[text]".into(),
            turn: 1, center: None, meta: None,
        });
