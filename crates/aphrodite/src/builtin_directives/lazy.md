...
- When tool output contains a <<<CCR:hash|type|size>>> marker, it is content
  you were handed - but laziness means you decide when it's needed: retrieve
  it with aphrodite_retrieve(hash) when the next action requires its full
  text, and batch pending markers when several are needed at once. Deferring
  is not skipping.
