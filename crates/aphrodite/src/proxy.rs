	...
	fn threshold_for(&self, ct:&str) -> usize {
		...
		match ct {
			...
			"code_rust" | "code_python" | "code_go" | "code_js" | "code" => {
				(base as f64 * self.code_multiplier()) as usize
			},
			"diff" | "git" => base * 2,
			"text" => base * 2,
			"tool_output" => base,
		...
		match chunk {
			Ok(b) => {
				if buf.len() + b.len() > max_bytes {
					return Err(format!("response body exceeded {} MB limit", max_bytes / (1024 * 1024)));
				}
...
pub(crate) mod tests {
	...
	fn test_handle_ccr_reload_applies_thresholds_to_state() {
		...
		let path = dir.join(format!(
			"aphrodite_reload_test_{}_{}.toml",
			std::process::id(),
			fnv1a_64(b"reload-test-salt")
		));
		std::fs::write(
			&path,
			r#"
...
"#,
	...
	fn test_response_cache_get_expires_past_ttl() {
		...
		state
			...
			.lock()
			.unwrap()
			.put(42, (std::time::Instant::now(), b"cached".to_vec()));
	...
	fn test_response_cache_get_hits_within_ttl() {
		...
		state
			...
			.lock()
			.unwrap()
			.put(7, (std::time::Instant::now(), b"cached".to_vec()));
	...
	fn test_compress_chat_completion_tool_call_arguments_pass_through_untouched() {
		...
		let new_content = response["choices"][0]["message"]["content"].as_str().unwrap();
		assert!(
			new_content.contains("<<<CCR:"),
			"expected message content to be compressed: {new_content}"
		);
		...
		assert_eq!(
			new_args, big_args,
			"tool-call arguments must pass through untouched, never marker-replaced"
		);
	}
	...
	fn test_execute_tool_relay_compress_small_content_also_stores_durably() {
		...
		let content = "tiny"; // well under INLINE_CCR_THRESHOLD (256B)
		let rt = tokio::runtime::Runtime::new().unwrap();
		let result = rt.block_on(execute_tool_relay(
			&state,
			"aphrodite_compress",
			&serde_json::json!({"content": content}),
		));
		...
		assert_eq!(
			durable.as_deref(),
			Some(content),
			"tiny content must also be durable, not inline-only"
		);
	}
