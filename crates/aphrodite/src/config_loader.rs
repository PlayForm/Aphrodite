...
impl Config {
    ...
    pub fn load() -> Self {
        let search_paths = vec![
            ...
            dirs::home_dir().unwrap_or_default().join(".hermes").join("aphrodite").join("aphrodite.toml"),
        ];
    ...
    pub fn apply_compression(&self, state: &mut crate::state::AphroditeState) {
        ...
        state.engine_threshold_pct = self.get_u64("APHRODITE_ENGINE_THRESHOLD_PCT", "compression", "engine_threshold_pct", 45);
        state.engine_min_msgs = self.get_usize("APHRODITE_ENGINE_MIN_MSGS", "compression", "engine_min_msgs", 8);
        state.engine_protect_first = self.get_usize("APHRODITE_ENGINE_PROTECT_FIRST", "compression", "engine_protect_first", 2);
        state.engine_protect_last = self.get_usize("APHRODITE_ENGINE_PROTECT_LAST", "compression", "engine_protect_last", 5);
        state.tool_threshold = self.get_usize("APHRODITE_TOOL_THRESHOLD", "compression", "tool_threshold", 4096);
        state.terminal_threshold = self.get_usize("APHRODITE_TERMINAL_THRESHOLD", "compression", "terminal_threshold", 1024);
...
mod tests {
    ...
    fn test_override() {
        ...
        assert_eq!(cfg.get_u64("APHRODITE_ENGINE_THRESHOLD_PCT", "compression", "engine_threshold_pct", 45), 90);
    }
}
