...
fn regex_match(line: &str, pattern: &str) -> Option<Vec<String>> {
    ...
    if pattern.contains(r"fn\s+(\w+)") {
        ...
        if lower.starts_with("fn ") || lower.starts_with("pub fn ") || lower.starts_with("async fn ") || lower.starts_with("pub async fn ") {
    ...
    if pattern.contains(r"function\s+(\w+)") {
        if lower.starts_with("export function ") || lower.starts_with("export async function ") || lower.starts_with("function ") {
            return Some(vec![line.to_string()]);
        }
    }
...
mod tests {
    ...
    fn test_reduce_build_errors() {
        // Use mid-line error/warning (prefix "error:" and "warning:" are intentionally excluded
        // because they're structured prefixes handled by summary extraction)
        let build = "   Compiling foo v0.1.0\n   process failed with error code 1\n   found a warning in the pipeline\n   Finished dev [unoptimized] target(s)\n";
    ...
    fn test_integration_json_minifies() {
        ...
        // This content IS >80 chars but after minification + header it may not save
