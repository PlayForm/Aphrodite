	...
	for i in (0..task_count).rev() {
		...
		match &task.status {
			...
			BgStatus::Done { exit_code } => {
				let hash_preview = task.output_preview.as_deref().unwrap_or("[output available]");
				crate::flow::push_nudge(
					state,
					&format!(
			...
			BgStatus::Failed { exit_code } => {
				let hash_preview = task.output_preview.as_deref().unwrap_or("[error output available]");
				crate::flow::push_nudge(
					state,
					&format!(
...
pub fn insert_bg_task(state: &mut AphroditeState, task_id: String, tool: String, command: String, turn: usize) {
	...
	while state.bg_tasks.len() >= MAX_BG_TASKS {
		...
		let stale_pos = state.bg_tasks.iter().position(|t| t.status == BgStatus::Stale);
		let done_pos = state
			.bg_tasks
			.iter()
...
mod tests {
	...
	fn test_should_background_slow_command() {
		let result = should_background(&"x".repeat(5000), Some("cargo build --release"));
		assert!(result.is_some(), "cargo build should be backgrounded");
	}

	#[test]
	fn test_should_not_background_small_output() {
		let result = should_background("short output", Some("cargo build"));
	...
	fn test_should_background_large_unknown_output() {
		...
		let result = should_background(&"x".repeat(10000), Some("some-slow-script.sh"));
		assert!(
			result.is_none(),
			"unknown command should NOT be backgrounded (slow-prefix check)"
		);
	}

	#[test]
	fn test_should_not_background_empty_command_small_output() {
		let result = should_background("small", None);
	...
	fn test_should_background_no_command_very_large_output() {
		...
		let result = should_background(&"x".repeat(10000), None);
		assert!(
			result.is_none(),
			"no-command content should not be backgrounded (needs known prefix)"
		);
	}
	...
	fn test_insert_bg_task_dedup() {
		...
		insert_bg_task(&mut state, "task1".into(), "terminal".into(), "cargo build".into(), 1);
		assert_eq!(state.bg_tasks.len(), 1);

		// Same ID, still running - should dedup.
		insert_bg_task(&mut state, "task1".into(), "terminal".into(), "cargo build".into(), 2);
		assert_eq!(state.bg_tasks.len(), 1);
	}
	...
	fn test_insert_bg_task_eviction_at_capacity() {
		...
		for i in 0..MAX_BG_TASKS + 1 {
			insert_bg_task(&mut state, format!("task{i}"), "terminal".into(), format!("cmd{i}"), i);
		}
		assert_eq!(state.bg_tasks.len(), MAX_BG_TASKS, "must not exceed MAX_BG_TASKS");
	}
	...
	fn test_render_bg_task_status_with_tasks() {
		let mut state = AphroditeState::default();
		insert_bg_task(&mut state, "task1".into(), "terminal".into(), "cargo build".into(), 5);
		insert_bg_task(&mut state, "task2".into(), "terminal".into(), "npm install".into(), 6);
	...
	fn test_check_bg_tasks_pushes_completion_nudge() {
		...
		let nudge = state.ephemeral_directives.last().unwrap();
		assert!(
			nudge.inline.as_deref().unwrap().contains("completed"),
	...
	fn test_multiple_rapid_polls_accumulate_output() {
		let mut state = AphroditeState::default();
		state.turn_counter = 5;
		insert_bg_task(&mut state, "t1".into(), "terminal".into(), "long-running".into(), 1);
	...
	fn test_poll_with_exit_code_completes_task() {
		...
		insert_bg_task(&mut state, "t1".into(), "terminal".into(), "npm install".into(), 1);
	...
	fn test_poll_with_text_exit_code_completes_task() {
		let mut state = AphroditeState::default();
		state.turn_counter = 5;
		insert_bg_task(&mut state, "t1".into(), "terminal".into(), "make build".into(), 1);
	...
	fn test_accumulated_output_capped_at_256kb() {
		let mut state = AphroditeState::default();
		state.turn_counter = 5;
		insert_bg_task(&mut state, "t1".into(), "terminal".into(), "very-verbose".into(), 1);
	...
	fn test_expire_stale_does_not_touch_recently_polled_tasks() {
		let mut state = AphroditeState::default();
		state.turn_counter = 20;
		insert_bg_task(&mut state, "active".into(), "terminal".into(), "recent-cmd".into(), 15);
	...
	fn test_check_bg_tasks_respects_2_nudge_cap() {
		...
		assert!(new_nudges <= 2, "must push at most 2 nudges, pushed {}", new_nudges);
	}
	...
	fn test_should_background_boundary_2047_bytes() {
		...
		let result = should_background(&content, Some("cargo build"));
		assert!(result.is_none(), "2047 bytes should be below the minimum threshold");
	}
	...
	fn test_should_background_no_command_large_output() {
		...
		assert!(
			result.is_none(),
			"no-command output should not be backgrounded (needs known prefix)"
		);
	}
