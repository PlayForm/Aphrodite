...
fn spawn_proxy() -> Proxy {
    ...
    loop {
        if std::net::TcpStream::connect(("127.0.0.1", TOKEN_PORT)).is_ok() { break; }
...
fn read_ema(port: u16) -> f64 {
    ...
    out.and_then(|v| v.get("compression_ratio_ema").and_then(|e| e.as_f64())).unwrap_or(0.0)
}
...
fn measure_threshold(port: u16, ct: &str) -> usize {
    ...
    let compressed = |size: usize| -> bool {
        ...
        let out = Command::new("curl")
            ...
            .output().ok()
            .and_then(|o| serde_json::from_slice::<serde_json::Value>(&o.stdout).ok())
            .and_then(|v| v.get("compression_ratio").and_then(|r| r.as_f64()))
            .map(|r| r > 1.05)
            .unwrap_or(false)
    };
    let mut lo = 256usize;
    ...
    while lo + 1 < hi {
        let mid = (lo + hi) / 2;
        if compressed(mid) { hi = mid; } else { lo = mid; }
    }
    hi
}
...
fn main() {
    ...
    let low = || -> String {
        (0u64..500).map(|i| format!("{:016x}", i.wrapping_mul(6364136223846793005))).collect()
    };
    ...
    for _ in 0..10 { ccr_create(proxy.port, &high()); }
    let ema_a = read_ema(proxy.port);
    eprintln!("  EMA after phase A: {:.2}x", ema_a);
    check(1, &format!("phase A: EMA > 5.0x (got {:.2}x)", ema_a), ema_a > 5.0, &mut failures);
    check(2, &format!("phase A: EMA < 500x (got {:.2}x)", ema_a), ema_a < 500.0, &mut failures);

    // ── Phase B: shock with incompressible payloads ───────────────────────
    eprintln!("\n[bench_04] Phase B: 10 low-ratio inserts");
    for _ in 0..10 { ccr_create(proxy.port, &low()); }
    let ema_b = read_ema(proxy.port);
    eprintln!("  EMA after phase B: {:.2}x", ema_b);
    check(3, &format!("phase B: EMA decayed below phase A ({:.2} < {:.2})", ema_b, ema_a),
          ema_b < ema_a, &mut failures);
    check(4, &format!("phase B: EMA still > 1.0x (got {:.2}x)", ema_b), ema_b > 1.0, &mut failures);
    ...
    check(5, &format!("phase C: EMA recovered above phase B ({:.2} > {:.2})", ema_c, ema_b),
          ema_c > ema_b, &mut failures);
    ...
    if ema_check > 20.0 {
        ...
        check(6,
            &format!("linter threshold <= 600 B even with EMA {:.1}x (got {} B)", ema_check, linter_thresh),
            linter_thresh <= 600,
            &mut failures);
        ...
        check(7,
            &format!("build_output threshold <= 600 B even with EMA {:.1}x (got {} B)", ema_check, build_thresh),
            build_thresh <= 600,
            &mut failures);
    } else {
    ...
    if failures > 0 { eprintln!("[bench_04] FAILED"); std::process::exit(1); }
    else { eprintln!("[bench_04] OK"); }
}
