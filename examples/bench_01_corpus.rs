...
//! Prints a per-label table and exits non-zero on any retrieve miss.
//!
...
fn corpus() -> Vec<Sample> {
    vec![
        ...
        Sample { label: "large_prose",
            content: "Lorem ipsum dolor. ".repeat(600) },                   // ~11 KB
        Sample { label: "rust_code",
            content: include_str!("bench_payloads/sample.rs").to_string() }, // ~3.5 KB
        Sample { label: "build_output",
            content: "   Compiling aphrodite v0.5.0\n".repeat(70)
                   + "    Finished release in 14.2s\n" },
        Sample { label: "linter_output",
            content: (0..50).map(|i| format!(
                "error[E0308]: mismatched types\n  --> src/lib.rs:{}:5\n   |\n{}|   expected `u64`\n",
                i * 3 + 1, " ".repeat(3))).collect::<String>() },
        Sample { label: "diff",
            content: format!(
                "diff --git a/src/proxy.rs b/src/proxy.rs\nindex abc..def 100644\n--- a/src/proxy.rs\n+++ b/src/proxy.rs\n{}",
                (0..100).map(|i| format!("-old line {}\n+new line {}\n", i, i)).collect::<String>()) },
        Sample { label: "json_tool",
        ...
        Sample { label: "error_output",
            content: format!(
                "thread 'main' panicked at 'index out of bounds'\nstack backtrace:\n{}",
                (0..35).map(|i| format!("  {}: some::module::fn_{} at src/lib.rs:{}\n", i, i, i*4+1)).collect::<String>()) },
        Sample { label: "log_output",
            content: (0..80).map(|i| format!(
                "2026-06-16T09:{:02}:{:02}Z [INFO] request id=req-{:04} elapsed={}ms\n",
                i / 60, i % 60, i, i * 7 + 1)).collect::<String>() },
        Sample { label: "unicode_cjk",
            content: "日本語テスト привет мир 🦀🔥 ".repeat(140) },              // ~4 KB mixed UTF-8
    ]
}
